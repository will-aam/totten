  ...
  return (
    <ResponsiveModal open={open} onOpenChange={onOpenChange} title="Vender Novo Pacote">
      <div className="flex flex-col gap-4 py-4 px-1">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={selectedClientId} onValueChange={setSelectedClientId} disabled={loadingClients || loading}>
              ...
              <SelectContent className="border border-border/50 bg-background shadow-xl rounded-2xl">
                {clients.map((c: any) => (
                  <SelectItem key={c.id} value={c.id} className="rounded-2xl py-2 font-medium">
                    {c.name}
                  </SelectItem>
                ))}
              </SelectContent>
        ...
        {currentTemplate && (
          <div className="bg-muted/30 p-4 rounded-2xl border border-border space-y-2 mt-2">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-row items-center justify-between rounded-2xl border border-border/50 p-3 bg-muted/20">
              ...
              <Select value={selectedMethod} onValueChange={setSelectedMethod} disabled={loading}>
                ...
                <SelectContent className="border border-border/50 bg-background shadow-xl rounded-2xl">
                  {paymentMethods
                    ...
                    .map((pm) => (
                      <SelectItem key={pm.id} value={pm.type} className="rounded-2xl py-2 font-medium">
                        {pm.name}
                      </SelectItem>
                    ))}
                </SelectContent>
          ...
          {!payUpfront && (
            <div className="flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
              <div className="flex flex-row items-center justify-between rounded-2xl border border-border/50 p-3 bg-amber-500/10 dark:bg-amber-500/5">
                <div className="space-y-0.5 pr-4">
