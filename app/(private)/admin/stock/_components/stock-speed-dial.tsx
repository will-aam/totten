...
import { Plus, Filter, FolderDownArrow, Package } from "@boxicons/react";
import { cn } from "@/lib/utils";

interface StockSpeedDialProps {
  onFilter: () => void;
  onExport: () => void;
  onNewItem: () => void;
...
export function StockSpeedDial({
  onFilter,
  onExport,
  onNewItem,
  showScrollTop,
  ...
  const actions = [
    { name: "Cadastrar", icon: <Package className="w-5 h-5" />, onClick: onNewItem },
    { name: "Exportar", icon: <FolderDownArrow className="w-5 h-5" />, onClick: onExport },
    { name: "Filtrar", icon: <Filter className="w-5 h-5" />, onClick: onFilter },
  ];

  return (
    <div
      className={cn(
        "fixed bottom-20 right-4 z-50 flex flex-col items-end transition-all duration-300 md:hidden",
