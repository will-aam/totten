...
export function ServiceEditModal({
  ...
  const handleToggleStatus = async (forceCascade: boolean = false) => {
    setLoading(true);
    try {
      const res = await toggleServiceStatus(
        service.id,
        service.active,
        forceCascade,
      );
      ...
      if (res.requiresConfirmation) {
        // Se a API pediu confirmação (tem pacotes vinculados), abrimos o modal secundário
        setConfirmCascade({
          show: true,
          message: res.error || "Tem certeza que deseja inativar?",
          pendingAction: () => handleToggleStatus(true), // A ação pendente é forçar a cascata
        });
        return;
      }
      ...
      if (res.success) {
        toast.success(
          service.active
            ? "Serviço e dependências desativados"
            : "Serviço ativado",
        );
        onSuccess();
        onOpenChange(false);
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              variant="destructive"
              onClick={async () => {
                await confirmCascade.pendingAction();
              }}
              disabled={loading}
              className="w-full sm:w-auto"
            >
