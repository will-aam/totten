// lib/server/services/totem/search.service.ts
import { getTenantPrisma } from "@/lib/prisma";

export class TotemSearchService {
  /**
   * Busca um cliente por CPF ou Telefone no contexto da organização.
   * Se encontrar apenas um agendamento válido para o dia, realiza o Auto Check-in.
   */
  static async searchAndProcess(
    value: string,
    mode: "CPF" | "PHONE",
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (agendamento.package_id) {
        ...
        await tx.appointment.update({
          where: { id: agendamento.id, organization_id: organizationId },
